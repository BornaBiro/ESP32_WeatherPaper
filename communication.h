...
class communication
{
    ...
    uint8_t saveDataToSD(struct sensorData *_s, struct data1StructHandle *_d1, struct data2StructHandle *_d2);
    int16_t getNumberOfEntries(time_t _epoch, uint8_t _indoor);
    uint8_t getOutdoorDataFromSD(time_t _epoch, int16_t _n, struct data1StructHandle *_d1, struct data2StructHandle *_d2);
    void getFileNameFromEpoch(char *_s, time_t _epoch);
    void getFolderPath(char *_s, time_t _epoch, uint8_t _indoor);
    void getOutdoorDataFromSD(struct data1StructHandle *_d1, struct data2StructHandle *_d2);
    ...
    const char *_outdoorDataEntry = "%d.;%ld;%02d.%02d.%04d;%02d:%02d:%02d;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%.1f;%.2f;%.2f;%.2f;%d;%.2f;";
    ...
    const char *_indoorDataEntry = "%d.;%ld;%02d.%02d.%04d;%02d:%02d:%02d;%.2f;%.2f;%.2f;%.2f;%d;%d;%d;%d";
